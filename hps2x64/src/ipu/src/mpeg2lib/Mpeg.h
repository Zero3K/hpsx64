...
struct decoder_t {
	...
	{
		// modification for hps2x64
		//uint mb_offset = ((uptr)&obj - (uptr)&mb8);
		uint mb_offset = ((u8*)&obj - (u8*)&mb8);
	...
	uint ipu0_data_input;		// amount of data in the input macroblock (in QWC)
	uint ipu0_idx_input;
	
	template< typename T >
	void SetInputTo( T& obj )
	{
		// modification for hps2x64
		//uint mb_offset = ((uptr)&obj - (uptr)&mb8);
		uint mb_offset = ((u8*)&obj - (u8*)&mb8);
		...
		ipu0_idx_input	= mb_offset / 16;
		ipu0_data_input	= sizeof(obj)/16;
	}
	...
	u64* GetIpuInputPtr()
	{
		// modification for hps2x64
		//return ((u128*)&mb8) + ipu0_idx;
		return ((u64*)&mb8) + ( ipu0_idx_input << 1 );
	}
	
	void AdvanceIpuInputBy(uint amt)
	{
		//pxAssertMsg(ipu0_data>=amt, "IPU FIFO Overflow on advance!" );
		ipu0_idx_input  += amt;
		ipu0_data_input -= amt;
...
extern void ALIGN ();
extern u64 PEEKBITS ( uint num );
extern bool GETWORD ();
extern bool BITSTREAM_INIT ();
